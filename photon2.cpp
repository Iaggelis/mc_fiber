...
int main(int argc, char **argv)
{
    ...
    auto filter1 = photons_load.Define("r", "x*x+y*y").Filter("r*r<=1.0", "Radius").Define("phi", genPhi).Define("alpha", "asin(r*sin(phi))").Define("theta", genTheta).Define("beta", "acos(sin(theta)*cos(alpha))").Filter([&](const double &b) { return b >= beta0; }, {"beta"}, "beta_cut");
